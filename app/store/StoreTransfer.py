...
sys.path.append('/users/jc/work/eda5_manage/api')
from models import *


def extract():
	instrument = Instrument.from_store('./instrumentstore.js')[0]
	questions = Question.from_store('./questionstore.js')
	answers = Answer.from_store('./answerstore.js')
...
def read_stores():
	...
	with file('./answerstore.js', 'r') as answers_file:
		answers_store = answers_file.read()

	answers_tree = parser.parse(answers_store)

	answers_data = answers_tree.children()[0].children()[0].children()[2].children()[1].children()[1].children()[2].children()[1]
	answers = obj2dict(answers_data)


	with file('./questionstore.js', 'r') as questions_file:
	...
	with file('./instrumentstore.js', 'r') as istore_file:
		istore = istore_file.read()
	istore_tree = parser.parse(istore)
	istore_data = istore_tree.children()[1].children()[0].children()[2].children()[2].children()[1].children()[2].children()[1]
